        ...
        <div class="group-data-[sidebar-size=lg]:ltr:md:ml-vertical-menu group-data-[sidebar-size=lg]:rtl:md:mr-vertical-menu group-data-[sidebar-size=md]:ltr:ml-vertical-menu-md group-data-[sidebar-size=md]:rtl:mr-vertical-menu-md group-data-[sidebar-size=sm]:ltr:ml-vertical-menu-sm group-data-[sidebar-size=sm]:rtl:mr-vertical-menu-sm pt-[calc(theme('spacing.header')_*_1)] pb-[calc(theme('spacing.header')_*_0.8)] px-4 group-data-[navbar=bordered]:pt-[calc(theme('spacing.header')_*_1.3)] group-data-[navbar=hidden]:pt-0 group-data-[layout=horizontal]:mx-auto group-data-[layout=horizontal]:max-w-screen-2xl group-data-[layout=horizontal]:px-0 group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:ltr:md:ml-auto group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:rtl:md:mr-auto group-data-[layout=horizontal]:md:pt-[calc(theme('spacing.header')_*_1.6)] group-data-[layout=horizontal]:px-3 group-data-[layout=horizontal]:group-data-[navbar=hidden]:pt-[calc(theme('spacing.header')_*_0.9)]">
            <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
                <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                    <div class="grow">
                        <h5 class="text-16">จัดการข้อมูลแบบฟอร์ม</h5>
                    </div>
                </div>
                <!-- Open Section  ############################################################## -->
                    <div class="card">
                        <div class="card-body">
                            <div class="shrink-0">                                        
                                <button aria-label="button" data-modal-target="AddMenuModal" type="button" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                    <i data-lucide="plus" class="inline-block size-4"></i> <span class="align-middle">เพิ่มแบบฟอร์ม</span>
                                </button>&emsp;
                                <a href="จัดการสายงาน.html">
                            ...
                            <table id="borderedTable" class="bordered group" style="width:100%">
                                ...
                                <tbody>
                                    <?php
                                        $no=0;
                                        $query_sheetmain = $conn->prepare("SELECT *,SUBSTRING(SHEET.SH_EFFECT, 9, 2) DAY,SUBSTRING(SHEET.SH_EFFECT, 6, 2) MONTH,SUBSTRING(SHEET.SH_EFFECT, 1, 4) REAL_YEAR,SUBSTRING(SUBSTRING(SHEET.SH_EFFECT, 1, 4), 3, 4) YEAR FROM SHEET LEFT JOIN LINEWORK ON LINEWORK.LW_ID=SHEET.LW_ID WHERE SHEET.LW_ID = :LW_ID AND NOT SH_STATUS='D'") or die($conn->error);
                                        $query_sheetmain->execute(array(":LW_ID" => $_GET['id']));
                                        while($rs_sheetmain = $query_sheetmain->fetch(PDO::FETCH_OBJ)) { 
                                            $no++;
                                    ?>                                        
                                        <tr align="center">
                                            <td align="center"><?php print "$no.";?></td>
                                            <td align="left"><?php echo $rs_sheetmain->SH_NAME; ?></td>
                                            <td align="center">
                                                <div class="flex justify-center gap-2">
                                                    <div class="edit">
                                                        <a href="ข้อมูลรายการตรวจสอบ-<?php echo $rs_sheetmain->SH_ID ?>-<?php echo $_GET['id'];?>.html">
                                            ...
                                            <td align="left"><?php echo $rs_sheetmain->SH_TIMES ?></td>
                                            <td align="left"><?php echo $rs_sheetmain->DAY ?>-<?php echo $rs_sheetmain->MONTH ?>-<?php echo $rs_sheetmain->YEAR+43 ?></td>
                                            <td align="center"><?php if($rs_sheetmain->SH_STATUS=="Y"){print "<img src='assets/images/check_true.gif' alt='pic' width='16' height='16'>";}else{print "<img src='assets/images/check_del.gif' alt='pic' width='16' height='16'>";}?></td>
                                            <td align="center">
                                                <div class="flex justify-center gap-2">
                                                    <div class="edit">
                                                        <a href="แก้ไขแบบฟอร์ม-<?php echo $rs_sheetmain->SH_ID ?>-<?php echo $_GET['id'];?>.html">
                                                    ...
                                                    <div class="remove">
                                                        <button aria-label="button" type="button" onclick="swaldelete_listmain('<?php print $rs_sheetmain->SH_CODE ?>','<?php print $no;?>','<?php echo $_GET['id'];?>')" class="py-1 text-xs text-white bg-red-500 border-red-500 btn hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:border-red-600 active:ring active:ring-red-100 dark:ring-custom-400/20 remove-item-btn">
...
<div id="AddMenuModal" modal-center="" class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-between p-4 border-b dark:border-zink-300/20">
            <h5 class="text-16">เพิ่มแบบฟอร์ม</h5>
        ...
        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
            <form name="form1" method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="param2" id="param2" placeholder="แก้ไขครั้งที่..." class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" autocomplete="off">
                </div>
                <div class="mb-3">
                    <label for="userId" class="inline-block mb-2 text-base font-medium">มีผลบังคับใช้</label>
                    <input type="date" name="param3" id="param3" autocomplete="off" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                </div>
                <div class="mb-3">
                    <label for="statusSelect" class="inline-block mb-2 text-base font-medium">สถานะ</label>
                ...
                <div class="flex justify-end gap-2 mt-4">    
                    ...
                    <input type="hidden" name="param5" id="param5" value="<?php echo $_GET['id']; ?>">
                    ...
                    <button aria-label="button" type="button" onclick="ManageListMain('ADD','')" class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">เพิ่มข้อมูล</button>
                </div>
            </form>
        </div>
    </div>
